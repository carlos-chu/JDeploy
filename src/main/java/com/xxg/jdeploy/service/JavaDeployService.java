...
public class JavaDeployService {
    ...
    public String deploy(String uuid) throws IOException {
        ...
        if (info != null) {
            ...
            // 打包
            String[] cmdArray = { "sh", shellFileFolder + "/package.sh", info.getUuid(), info.getUrl(), basePath,
                    String.valueOf(info.getType()) };
            sb.append(ShellUtil.exec(cmdArray));
            String finalName = getFinalName(info.getUuid());
            if (finalName != null) {
                // 启动程序
                sb.append(ShellUtil.exec("sh " + shellFileFolder + "/start.sh " + info.getUuid() + " " + finalName
                        + " " + basePath));
            } else {
                sb.append("打包失败");
            }
